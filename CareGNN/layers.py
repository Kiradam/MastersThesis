...
class InterAgg(nn.Module):
    ...
    def __init__(self, features, feature_dim,
        ...
        for i in range(self.num_rel):
            self.intra_agg_list.append(intraggs[i])
            self.intra_agg_list[i].cuda = cuda
        ...
        self.thresholds = [0.5] * self.num_rel
    ...
    def forward(self, nodes, labels, train_flag=True):
        ...
        union_neigh_list = []
        for i in range(self.num_rel):
            union_neigh_list.append(set.union(*to_neighs[i]))
        unique_nodes = set.union(*union_neigh_list, set(nodes))
        ...
        r_result_scores = []
        for i in range(self.num_rel):
            r_list.append([list(to_neigh) for to_neigh in to_neighs[i]])
            r_scores.append([batch_scores[itemgetter(*to_neigh)(id_mapping), :].view(-1, 2) for to_neigh in r_list[i]])
            r_sample_list.append([math.ceil(len(neighs) * self.thresholds[i]) for neighs in r_list[i]])
            feature, score = self.intra_agg_list[i].forward(nodes, r_list[i], center_scores, r_scores[i],
                                                            r_sample_list[i])
            r_features.append(feature)
            r_result_scores.append(score)

        # concat the intra-aggregated embeddings from each relation
        neigh_feats = torch.cat(r_features, dim=0)
        ...
        if self.RL and train_flag:
            relation_scores, rewards, thresholds, stop_flag = rl_module(r_result_scores,
                                                                        self.relation_score_log,
                                                                        labels,
                                                                        self.thresholds,
...
def rl_module(scores, scores_log, labels, thresholds, batch_num, step_size):
    """
    The reinforcement learning module.
